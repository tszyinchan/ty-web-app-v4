-- CG: Package look — color (default) or mono (B&W).
-- Additive. Does NOT drop tables. Run in the Supabase SQL editor.
-- Replaces tyapp_cg_get_output_by_token so Output also returns `look`.

alter table public.tyapp_cg_package
  add column if not exists look text not null default 'color';

alter table public.tyapp_cg_package
  drop constraint if exists tyapp_cg_package_look_check;

alter table public.tyapp_cg_package
  add constraint tyapp_cg_package_look_check
